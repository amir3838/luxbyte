// ===============================
// restaurant/restaurant-dashboard.js — wires UI to API
// ===============================
import * as api from './api/restaurant.js';
...
export class RestaurantDashboard {
  ...
  cacheEls() {
    ...
    this.$menuList = document.querySelector('#menu .menu-list');
    ...
    this.$kpiMenu = this.$overview?.querySelector('.stat-card:nth-child(3) .stat-value');
  ...
  bind() {
    ...
    // Menu actions
    document.querySelector('#menu .btn-primary')?.addEventListener('click', () => this.openAddMenuItemModal());
  ...
  async init() {
    try {
      ...
      console.error('❌ Restaurant Dashboard init error:', error);
  ...
  async refreshKpis() {
    try {
      ...
      this.$kpiMenu.textContent = kpis.menu_items;
  ...
  async refreshMenu() {
    ...
    this.$menuList.querySelectorAll('[data-generated]')?.forEach(el => el.remove());
  ...
  createMenuItemCard(item) {
    ...
    card.innerHTML = `
      <div class="menu-item-image">
        ${item.image_url ? `<img src="${item.image_url}" alt="${item.name}" loading="lazy">` : '<div class="no-image">لا توجد صورة</div>'}
      </div>
      <div class="menu-item-info">
        <h4>${item.name}</h4>
        <p class="description">${item.description || 'لا يوجد وصف'}</p>
        <div class="menu-item-details">
          <span class="price">${DashboardUtils.formatCurrency(item.price)}</span>
          <span class="category">${item.category || 'عام'}</span>
          <span class="status ${item.available ? 'available' : 'unavailable'}">${item.available ? 'متوفر' : 'غير متوفر'}</span>
        </div>
      </div>
      <div class="menu-item-actions">
        <button class="btn btn-sm btn-secondary" data-action="view" data-id="${item.id}">عرض</button>
        <button class="btn btn-sm btn-primary" data-action="edit" data-id="${item.id}">تعديل</button>
        <button class="btn btn-sm btn-danger" data-action="delete" data-id="${item.id}">حذف</button>
      </div>
    `;

    this.bindMenuItemActions(card, item);
    return card;
  }

  bindMenuItemActions(card, item) {
    card.querySelector('[data-action="view"]')?.addEventListener('click', () => this.viewMenuItem(item));
    card.querySelector('[data-action="edit"]')?.addEventListener('click', () => this.openEditMenuItemModal(item));
    card.querySelector('[data-action="delete"]')?.addEventListener('click', () => this.deleteMenuItem(item.id));
  }
  ...
  openAddMenuItemModal() {
    DashboardUtils.showError('نافذة إضافة عنصر قائمة جديد - قيد التطوير');
  }

  openEditMenuItemModal(item) {
    DashboardUtils.showError(`نافذة تعديل عنصر القائمة: ${item.name} - قيد التطوير`);
  }

  viewMenuItem(item) {
    DashboardUtils.showError(`عرض تفاصيل عنصر القائمة: ${item.name} - قيد التطوير`);
  }
  ...
  async deleteMenuItem(itemId) {
    ...
    try {
      ...
      DashboardUtils.showSuccess('تم حذف العنصر بنجاح');
      await this.refreshMenu();
      await this.refreshKpis();
    } catch (error) {
      DashboardUtils.showError('فشل حذف العنصر: ' + error.message);
    }
  }
}
