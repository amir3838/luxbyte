...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const [docType, fileData] of Object.entries(documents)) {
      if (fileData && fileData.file) {
        try {
          ...
          const fileName = `${user_id}/clinic/documents/${docType}_${timestamp}_${randomString}.${fileExtension}`;
    ...
    // Log the clinic documents upload
    await supabase
      .from('audit_logs')
      .insert([{
        user_id,
        action: 'CLINIC_DOCUMENTS_UPLOADED',
        table_name: 'clinic_documents',
        record_id: user_id,
        new_values: {
    ...
    return res.status(200).json({
      success: true,
      message: 'Clinic documents uploaded successfully',
      documents: uploadedDocuments
    });

  } catch (error) {
    console.error('Clinic documents upload error:', error);
