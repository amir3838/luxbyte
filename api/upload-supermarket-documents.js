...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const [docType, fileData] of Object.entries(documents)) {
      if (fileData && fileData.file) {
        try {
          ...
          const fileName = `${user_id}/supermarket/documents/${docType}_${timestamp}_${randomString}.${fileExtension}`;
    ...
    // Log the supermarket documents upload
    await supabase
      .from('audit_logs')
      .insert([{
        user_id,
        action: 'SUPERMARKET_DOCUMENTS_UPLOADED',
        table_name: 'supermarket_documents',
        record_id: user_id,
        new_values: {
    ...
    return res.status(200).json({
      success: true,
      message: 'Supermarket documents uploaded successfully',
      documents: uploadedDocuments
    });

  } catch (error) {
    console.error('Supermarket documents upload error:', error);
