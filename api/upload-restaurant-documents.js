...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const [docType, fileData] of Object.entries(documents)) {
      if (fileData && fileData.file) {
        try {
          ...
          const fileName = `${user_id}/restaurant/documents/${docType}_${timestamp}_${randomString}.${fileExtension}`;
    ...
    // Log the restaurant documents upload
    await supabase
      .from('audit_logs')
      .insert([{
        user_id,
        action: 'RESTAURANT_DOCUMENTS_UPLOADED',
        table_name: 'restaurant_documents',
        record_id: user_id,
        new_values: {
    ...
    console.error('Restaurant documents upload error:', error);
